...
class Farmer{
  ...
  //Accept Buy Request
   acceptRequest({required cropId, required buyerId, required buyerType, required requiredQuantity})async {
     ...
     await _firestore
         .collection('crops').doc(cropId).collection('buyRequests').doc(
         availableQuantity).delete();
     ...
     _sellCrop(buyerId: buyerId, buyerType: buyerType, amount: amount);
   }

   //sellCrop
   _sellCrop({required buyerId, required buyerType, required amount})async
   {
    final _transaction =  Transactions(senderId: buyerId, senderType: buyerType, receiverId: id, receiverType: 'farmer', amount: amount, farmerId: id);
